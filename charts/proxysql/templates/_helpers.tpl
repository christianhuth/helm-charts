...
*/}}
{{/*
ProxySQL configuration content for proxysql.cnf
*/}}
{{- define "proxysql.config" -}}
admin_variables=
{
...
mysql_variables=
{
  ...
  autocommit_false_is_transaction={{ .Values.proxysql.mysql.autocommitFalseIsTransaction }}
...
(
  ...
  {{- range $index, $element := .Values.proxysql.mysql.servers }}
  {
    ...
    use_ssl={{ (int $element.use_ssl) | default 0 }}
...
(
  ...
  {{- range $index, $element := .Values.proxysql.mysql.users }}
  {
    ...
    {{- if eq (toString $element.frontend) "0" }}
    frontend={{ toString $element.frontend }}
    {{- end }}
    {{- if eq (toString $element.backend) "0" }}
    backend={{ toString $element.backend }}
    {{- end }}
