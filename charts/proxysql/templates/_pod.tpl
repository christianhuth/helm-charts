...
spec:
  ...
  volumes:
    - name: proxysql-config
      {{- if or .Values.proxysql.secret.existingSecret .Values.proxysql.secret.enabled }}
      secret:
        secretName: {{ .Values.proxysql.secret.existingSecret | default (include "proxysql.fullname" .) }}
      {{- else }}
      configMap:
